...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt', variants='', hmoe_rank='', amglora_rank=''):
    try:
    ...
    seq_txt = seq_name
    save_folder = f'./RGBE_workspace/{variants}/{dataset_name}/'
    ...
    if script_name == 'seatrack':
        print(yaml_name)
        params = rgbe_prompt_params.parameters(yaml_name, epoch, variants)
        params.cfg.MODEL.HMOE_RANK = int(hmoe_rank)
        params.cfg.MODEL.AMGLORA_RANK = int(amglora_rank)
        ostrack = SEATrack(params)  # "VisEvent"
        tracker = SEATrack_RGBE(tracker=ostrack)
