...
import lib.test.parameter.seatrack as rgbt_prompt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt', variants='', hmoe_rank='', amglora_rank=''):
    ...
    if script_name == 'seatrack':
        params = rgbt_prompt_params.parameters(yaml_name, epoch, variants)
        params.cfg.MODEL.HMOE_RANK = int(hmoe_rank)
        params.cfg.MODEL.AMGLORA_RANK = int(amglora_rank)
        mmtrack = SEATrack(params)  # "GTOT" # dataset_name
        tracker = SEATrack_RGBT(tracker=mmtrack)
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            # track
            image = get_x_frame(rgb_path, T_path, dtype=getattr(params.cfg.DATA,'XTYPE','rgbrgb'))
            frame = os.path.splitext(os.path.basename(rgb_path))[0]
            region, confidence = tracker.track(image, dataset_name, seq_name)
            # xywh
...
class SEATrack_RGBT(object):
    ...
    def track(self, img_RGB, dataset_name, seq_name):
        '''TRACK'''
        outputs = self.tracker.track(img_RGB, dataset_name, seq_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threads', default=30, type=int, help='Number of threads')
    ...
    parser.add_argument('--variants', default='', type=str, help='specific exp variants')
    parser.add_argument('--hmoe_rank', default='4', type=str, help='specific exp variants')
    parser.add_argument('--amglora_rank', default='8', type=str, help='specific exp variants')
    ...
    if args.mode == 'parallel':
        sequence_list = [(s, seq_home, dataset_name, args.yaml_name, args.num_gpus, args.epoch, args.debug, args.script_name, args.variants, args.hmoe_rank, args.amglora_rank) for s in seq_list]
