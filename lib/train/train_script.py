...
from lib.train.actors import SEATrackActor
# for import modules
import importlib
...
def run(settings):
    settings.description = 'Training script for seatrack'
    ...
    if settings.script_name == "seatrack":
        ...
        objective = {'giou': giou_loss, 'l1': l1_loss, 'focal': focal_loss, 'cls': BCEWithLogitsLoss(), 'ce': KLDivLoss(reduction='batchmean')}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'focal': 1., 'cls': 1.0}
        actor = SEATrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
