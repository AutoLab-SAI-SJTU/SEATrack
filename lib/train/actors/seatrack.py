...
class SEATrackActor(BaseActor):
    """ Actor for training seatrack models """
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        # ce_r2dte = []
        # for i in [11]:
        #     ce_dte2r_i = self.objective['ce'](F.log_softmax(pred_dict['attns'][i][1], dim=-1), F.softmax(pred_dict['attns'][i][0], dim=-1))
        #     ce_dte2r.append(ce_dte2r_i)
        #     ce_r2dte_i = self.objective['ce'](F.log_softmax(pred_dict['attns'][i][0], dim=-1), F.softmax(pred_dict['attns'][i][1], dim=-1))
        #     ce_r2dte.append(ce_r2dte_i)
        # ce_dte2r = sum(ce_dte2r) / len(ce_dte2r)
        # ce_r2dte = sum(ce_r2dte) / len(ce_r2dte)
        # ce_loss = 0.7 * ce_dte2r + 0.3 * ce_r2dte
        # loss = 0.9 * loss + 0.1 * ce_loss
        if return_status:
            # status for log
