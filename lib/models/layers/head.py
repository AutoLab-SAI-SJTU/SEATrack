...
class CenterPredictor(nn.Module, ):
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        max_score, idx = torch.max(score_map_ctr.flatten(1), dim=1, keepdim=True) # score_map_ctr: (1, 1, 16, 16)
        idx_y = idx // self.feat_sz # 16 
        idx_x = idx % self.feat_sz # 16

        idx = idx.unsqueeze(1).expand(idx.shape[0], 2, 1) # (1, 2, 1)
        size = size_map.flatten(2).gather(dim=2, index=idx) # (1, 2, 16, 16)->(1, 2, 256)->(1, 2, 1)
        offset = offset_map.flatten(2).gather(dim=2, index=idx).squeeze(-1) # (1, 2, 16, 16) -> (1, 2, 256)->(1, 2)
